...
from actnn_aq.layers import QLinear # AISYS ADDED
...
class TensorVM(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        ...
        if self.actnn:
            self.basis_mat = QLinear(self.app_n_comp * 3, self.app_dim, bias=False, device=device)
...
class TensorVMSplit(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        ...
        if self.actnn:
            self.basis_mat = QLinear(sum(self.app_n_comp), self.app_dim, bias=False).to(device)
    ...
    def compute_densityfeature(self, xyz_sampled):
        ...
        sigma_feature = torch.zeros((xyz_sampled.shape[0],), device=xyz_sampled.device, dtype=xyz_sampled.dtype)
...
class TensorCP(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        ...
        if self.actnn:
            self.basis_mat = QLinear(self.app_n_comp[0], self.app_dim, bias=False).to(device)
